...
def run_processes(server_args,i, *client_args):
...
def suite(parties, expr, expected,i):
    participants = list(parties.keys())

    prot = ProtocolSpec(expr=expr, participant_ids=participants)
    clients = [(name, prot, value_dict) for name, value_dict in parties.items()]

    results = run_processes(participants, i, *clients)
...
def test_suite10():
 ...
 #   suite(parties, expr, expected,10)
